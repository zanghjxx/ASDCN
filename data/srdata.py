...
class SRData(data.Dataset):
        ...
        if args.ext == 'img' or benchmark:
            ...
            if args.ext.find('reset') >= 0:
                ...
                for v in self.images_hr:
                    hr = imageio.imread(v)
                    name_sep = v.replace(self.ext, '.pt')
                    np.save(name_sep, hr)
            ...
            self.images_hr = [
                v.replace(self.ext, '.pt') for v in self.images_hr
            ]
            self.images_lr = [
                [v.replace(self.ext, '.pt') for v in self.images_lr[i]]
    ...
    def _load_file(self, idx):
        ...
        if self.args.ext == 'img' or self.benchmark:
            ...
            filename = hr
            with open(hr, 'rb') as _f: hr = np.load(_f,allow_pickle=True)[0]['image']
            with open(lr, 'rb') as _f: lr = np.load(_f,allow_pickle=True)[0]['image']
        else:
            filename = str(idx + 1)
